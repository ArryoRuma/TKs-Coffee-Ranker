...
<template>
  <UCard>
    <div class="space-y-6">
      <div class="space-y-1">
        <h1 class="font-semibold text-stone-900">{{ title }}</h1>
        <p class="text-sm text-stone-500">{{ description }}</p>
      </div>

      <button
        class="inline-flex w-full items-center justify-center gap-2.5 rounded-md bg-stone-900 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-stone-700"
        @click="emit('github')"
      >
      ...
      <p class="text-center text-xs text-stone-500">
        {{ secondaryPrefix }}
        <NuxtLink :to="secondaryTo" class="underline hover:text-stone-800">
          {{ secondaryLabel }}
        </NuxtLink>
      </p>
    </div>
